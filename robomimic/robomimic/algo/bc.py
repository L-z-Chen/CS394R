...
def gmm_dist_to_var(dists):
    probs = dists.mixture_distribution.probs.squeeze()
    variance = torch.mean(dists.component_distribution.variance.squeeze(), dim=-1)
    mean_var = torch.sum(probs * variance, dim=1)

    if len(mean_var.shape) > 1:
        mean_var = torch.sum(mean_var, dim=1)
...
class BC_Gaussian(BC):
    ...
    def train_on_batch(self, batch, epoch, validate=False, var_stat=False):
        """
        Training on a single batch of data.
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            info = super(BC, self).train_on_batch(batch, epoch, validate=validate)
            predictions, batch_mean_var = self._forward_training(batch)
            losses = self._compute_losses(predictions, batch)
            info["predictions"] = TensorUtils.detach(predictions)
            info["losses"] = TensorUtils.detach(losses)

            # compute losses for demos with different quaities
            if var_stat:
                losses_list = -predictions["log_probs"]
                info['losses_list'] = losses_list.detach()
        ...
        if var_stat:
            return info, TensorUtils.detach(batch_mean_var)
        else:
            return info
    ...
    def _forward_training(self, batch):
        ...
        batch_mean_var = gmm_dist_to_var(dists)

        predictions = OrderedDict(
            log_probs=log_probs,
        )
        return predictions, batch_mean_var
