...
def load_data_for_training(config, obs_keys, dataset_path=None):
    ...
    if config.experiment.validate:
        ...
        train_dataset = dataset_factory(config, obs_keys, filter_by_attribute=filter_by_attribute,dataset_path=dataset_path)
        valid_dataset = None
...
def run_epoch(model, data_loader, epoch, validate=False, num_steps=None, var_stat=False, var_keys=None, return_non_avg=False):
    """
    ...
    Args:
        ...
        var_stat (bool): whether to output Gaussian model variance stat or not.

        var_keys (list): filter keys to compute variance state upon.
    ...
    if var_stat:
        key_var_stats = {var_key: [] for var_key in var_keys}
        key_loss_stats = {var_key: [] for var_key in var_keys}
    ...
    for _ in LogUtils.custom_tqdm(range(num_steps)):
        ...
        t = time.time()
        if var_stat:
            info, batch_mean_var = model.train_on_batch(input_batch, epoch, validate=validate, var_stat=True)
        else:
        ...
        # compute action variance statistics
        if var_stat:
            for idx, mask_names in enumerate(batch['mask_names']):
                for var_key in var_keys:
                    if var_key in mask_names:
                        key_var_stats[var_key].append(batch_mean_var[idx].cpu().item())
                        key_loss_stats[var_key].append(info['losses_list'][idx].cpu().item())
    ...
    if var_stat:
        for var_key, var_states in key_var_stats.items():
            save_name = '{}_mean_action_var'.format(var_key)
            step_log_all[save_name] = np.mean(var_states)

        for loss_key, loss_states in key_loss_stats.items():
            save_name = '{}_loss'.format(loss_key)
            step_log_all[save_name] = np.mean(loss_states)
    ...
    # return all losses before average if requested
    if return_non_avg:
        return key_loss_stats
